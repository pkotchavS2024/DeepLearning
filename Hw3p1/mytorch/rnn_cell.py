...
class RNNCell(object):
    ...
    def __init__(self, input_size, hidden_size, autograd_engine, act_fn=Tanh):
        ...
        self.ih = Linear(input_size, hidden_size, self.autograd_engine)
        self.hh = Linear(hidden_size, hidden_size, self.autograd_engine)
    ...
    def forward(self, x, h_prev_t, scale_hidden=None):
        ...
        if scale_hidden is not None:
            # Scale the hidden transformation
            scaled_hidden_transform = scale_hidden * hidded_transform

            # Register the scaled operation for autograd tracking
            self.autograd_engine.add_operation(
                inputs=[scale_hidden, hidded_transform],
                output=scaled_hidden_transform,
                gradients_to_update=[None, None],
                backward_operation=mul_backward
            )
        else:
            scaled_hidden_transform = hidded_transform
            # self.autograd_engine.add_operation(
            #     inputs=[hidded_transform],
            #     output=scaled_hidden_transform,
        ...
        total_transform = input_transform + scaled_hidden_transform
        self.autograd_engine.add_operation(
                inputs=[input_transform, scaled_hidden_transform],
