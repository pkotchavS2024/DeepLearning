...
def ctc_loss_backward(grad, logits, input_lengths, gammas, extended_symbols):
    ...
    for batch_itr in range(B):
        ...
        # <---------------------------------------------
        T_batch = input_lengths[batch_itr] 
        gamma_batch = gammas[batch_itr]  
        _ , S = gamma_batch.shape
        extended_seq = extended_symbols[batch_itr]  
        logit_batch = logits[:input_lengths[batch_itr],batch_itr,:]
        print(gamma_batch.shape)
        print(extended_seq.shape)
        print(logit_batch.shape)

        for t in range(T_batch):
            for s in range(S):
                dY[t,batch_itr,extended_seq[s]] -= grad*gamma_batch[t,s] / logit_batch[t,extended_seq[s]]
