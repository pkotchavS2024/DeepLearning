...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        # <---------------------
        self.convolutional_layers = [Conv1d(in_channels = num_input_channels if i == 0 else num_channels[i-1], 
                                            out_channels = num_channels[i],
                                            kernel_size = kernel_sizes[i],
                                            stride = strides[i]) for i in range(self.nlayers)]
        self.flatten = Flatten()
        conv_outwidth = -1
        for i in range(self.nlayers):
            if i == 0:
                conv_outwidth = (input_width - kernel_sizes[i]) // strides[i] + 1
            else: 
                conv_outwidth = (conv_outwidth - kernel_sizes[i]) // strides[i] + 1
        
        # Based on calculated conv_output_width of the last convolution layer, the flatten dimension is simply number of channels * the width of the output
        # which then corresponds to in_feats of Linear layer
        self.linear_layer = Linear( num_channels[2] * conv_outwidth, num_linear_neurons)  
    ...
    def backward(self, labels):
        ...
        # Back through linear and flatten
        grad = self.linear_layer.backward(grad)
        grad = self.flatten.backward(grad)

        # Backward through convolutional layers in reverse order
