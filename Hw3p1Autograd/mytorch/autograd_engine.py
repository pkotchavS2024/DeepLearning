...
class Autograd:
    ...
    def backward(self, divergence):
        ...
        for i, operation in enumerate(reversed(self.operation_list)):
            if i == 0:
                grad_propagate = divergence
            else:
                grad_propagate = self.gradient_buffer.get_param(operation.output)  # ***
            
            inp_grads = operation.backward_operation(grad_propagate, *operation.inputs)


            if len(operation.inputs) == 1:
                if operation.gradients_to_update[0] is None:
                    self.gradient_buffer.update_param(operation.inputs[0], inp_grads)
                else:
                    operation.gradients_to_update[i] += inp_grads
            else:
                for i, input in enumerate(operation.inputs):
                    if operation.gradients_to_update[i] is None:
                        self.gradient_buffer.update_param(input, inp_grads[i])
                    else:
                        operation.gradients_to_update[i] += inp_grads[i]
